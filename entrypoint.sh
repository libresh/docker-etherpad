...
if [ "$ETHERPAD_DB_USER" = 'root' ]; then
  : ${ETHERPAD_DB_PASSWORD:=$DB_ENV_MYSQL_ROOT_PASSWORD}
fi
...
for ((i=0;i<10;i++))
do
  DB_CONNECTABLE=$(mysql -uroot -p${ETHERPAD_DB_PASSWORD} -hdb -e 'status' >/dev/null 2>&1; echo "$?")
...
if [ "$RESULT" != $ETHERPAD_DB_NAME ]; then
  ...
  mysql -u${ETHERPAD_DB_USER} -p${ETHERPAD_DB_PASSWORD} -hdb \
